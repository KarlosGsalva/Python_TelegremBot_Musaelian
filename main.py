...
class TicTacToe:
    def __init__(self):
        self.player1 = f"{Fore.YELLOW}X{Style.RESET_ALL}".center(12)
        self.player1_turn = False
        self.player2 = f"{Fore.GREEN}O{Style.RESET_ALL}".center(12)
    ...
    def _mistake_input_warning():
        text = (f"{Fore.LIGHTCYAN_EX}\nВы ввели некоректное число, напомним, координаты должны быть в "
                f"диапазоне от 0 до 2 включительно\n"
                f"также следует учесть, чтобы поле не было занято Вами или соперником ;){Style.RESET_ALL}")
        return text

    @staticmethod
    def _winner_warning(player):
        text = f"{Fore.BLUE}\nИтак у нас появился победитель, Игрок {player}, поздравляем ;) !{Style.RESET_ALL}"
        return text
    ...
    @staticmethod
    def _hope_declare():
        return f"{Fore.LIGHTMAGENTA_EX}\nОк, тогда до встречи, надеюсь, увидимся снова ;){Style.RESET_ALL}"
