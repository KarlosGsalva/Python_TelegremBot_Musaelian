...
async def _make_callback_inline_buttons(mode: str) -> list | None:
    try:
        ...
        for note in files:
            if mode == 'delete':
                button = InlineKeyboardButton(text=f"Удалить {note[:-9]}",
                                              callback_data=f"delete_{note}")
            elif mode == 'read':
                button = InlineKeyboardButton(text=f"Прочитать {note[:-9]}",
                                              callback_data=f"read_{note}")
            elif mode == 'edit':
                button = InlineKeyboardButton(text=f"Редактировать {note[:-9]}",
                                              callback_data=f"edit_{note}")
