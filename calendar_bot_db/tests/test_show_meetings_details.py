...
async def test_cmd_8_show_user_meetings_details(dp, bot):
    ...
    # Вывод оставшихся элементов в очередях
    print("Remaining requests in queue:")
    while bot.session.requests:
        print(bot.session.requests.popleft())

    print("Remaining responses in queue:")
    while bot.session.responses:
        print(bot.session.responses.popleft())

    # Очистка очередей после теста
    bot.clear_queues()