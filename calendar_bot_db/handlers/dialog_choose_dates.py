...
async def select_event_date(
    ...
    try:
        ...
        await callback.message.answer(
            WTEXT["request_event_time"], reply_markup=kb.time_keyboard()
        )
...
async def select_meeting_date(
    ...
    try:
        ...
        # Запрашиваем время события, выдаем клавиатуру, меняем state
        await callback.message.answer(
            WTEXT["request_meeting_time"], reply_markup=kb.time_keyboard()
        )
...
set_calendar_window_for_event = Window(
    Const(WTEXT["request_event_date"]),
    set_event_date,
    state=FSMCreateEvent.fill_event_date,
)

set_calendar_window_for_meeting = Window(
    Const(WTEXT["request_meeting_date"]),
    set_meeting_date,
    state=FSMCreateMeeting.fill_meeting_date,
)

edit_event_date_calendar_window = Window(
    Const(WTEXT["request_event_date"]),
    edit_calendar_date,
    state=FSMEditEvent.edit_event_date,
)
