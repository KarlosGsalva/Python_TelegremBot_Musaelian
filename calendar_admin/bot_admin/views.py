...
def show_user_calendar(request):
    ...
    combined_list = sorted(
        events + meetings,
        key=lambda x: (
            x.event_date if isinstance(x, Event) else x.date,
            x.event_time if isinstance(x, Event) else x.time,
        ),
    )
...
def export_events_csv(request):
    ...
    try:
        ...
        return HttpResponse(
            status=500, content="An error occurred while exporting events"
        )
...
def export_events_json(request):
    try:
        ...
        return HttpResponse(
            status=500, content="An error occurred while exporting json events"
        )
